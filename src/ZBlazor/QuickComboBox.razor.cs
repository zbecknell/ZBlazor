...
namespace ZBlazor
{
		...
		where TItem : class
	{
		...
		/// <summary>
		/// When the type of TItem is not a string, this denotes the field name that should be used to display the item text. Defaults to <c>"Text"</c>.
		/// </summary>
		[Parameter] public string? TextField { get; set; } = "Text";

		/// <summary>
		/// Any additional class(es) to apply to the actual input box. Overrides <see cref="QuickAutocompleteGlobalProperties.Class"/>.
		/// </summary>
		[Parameter] public string? Class { get; set; }

		/// <summary>
		/// Any additional class(es) to apply to the outer div containing the input. Overrides <see cref="QuickAutocompleteGlobalProperties.ContainerClass"/>.
		/// </summary>
		[Parameter] public string? ContainerClass { get; set; }

		/// <summary>
		/// Any additional class(es) to apply to each item in the items list. Overrides <see cref="QuickAutocompleteGlobalProperties.ItemClass"/>.
		/// </summary>
		[Parameter] public string? ItemClass { get; set; }

		/// <summary>
		/// The width of the component. Defaults to <c>"300px"</c>.
		/// </summary>
		[Parameter] public string Width { get; set; } = "300px";

		/// <summary>
		/// When true, the list will open when the component gains focus. Defaults to <c>true</c>.
		/// </summary>
		...
		/// <summary>
		/// When true, highlights matches in the list. Defaults to <c>true</c>.
		/// </summary>
		[Parameter] public bool HighlightMatches { get; set; } = true;

		/// <summary>
		/// When true, clears the input value when the Escape key is pressed and the input has focus. Defaults to <c>true</c>.
		/// </summary>
		[Parameter] public bool ClearOnEscape { get; set; } = true;

		/// <summary>
		/// When true, a clear button will show when the input has a value. Defaults to <c>true</c>.
		/// </summary>
		[Parameter] public bool ShowClearButton { get; set; } = true;

		/// <summary>
		/// When true, the input in cleared when a value is selected. Defaults to <c>false</c>.
		/// </summary>
		...
		/// <summary>
		/// When populated, the matcher will also check other named fields on <typeparamref name="TItem"/> for a filter match.
		/// </summary>
		[Parameter] public IEnumerable<string>? OtherMatchFields { get; set; }

		/// <summary>
		/// When true, shows the name and value of the "other match" fields when matched. Defaults to <c>true</c>. Note that if you use an <see cref="ItemTemplate"/> you need to pass a value for <see cref="SearchItem{TDataObject}.GetDisplayText(bool, bool)"/> if you don't want other matches to show.
		/// </summary>
		...
		/// <summary>
		/// When true, results will be ordered with shorter values first. Takes precedence above <see cref="PrioritizePrimaryMatch"/>. Defaults to <c>false</c>.
		/// </summary>
		[Parameter] public bool PrioritizeShorterValues { get; set; } = false;

		/// <summary>
		/// When other (non-primary) fields match, sort primary matches to the top always. Defaults to <c>false</c>.
		/// </summary>
		[Parameter] public bool PrioritizePrimaryMatch { get; set; } = false;

		/// <summary>
		/// When true, input not matching any search item is allowed, otherwise input is cleared when not matching any search items. Defaults to <c>true</c>.
		/// </summary>
		[Parameter] public bool AllowCustomValues { get; set; } = true;

		/// <summary>
		/// The type of the input element. Defaults to <c>"text"</c>.
		/// </summary>
		...
		/// <summary>
		/// When true, a null value will be passed to <see cref="ValueChanged"/>. Defaults to <c>true</c>.
		/// </summary>
		[Parameter] public bool EmitNullOnInputClear { get; set; } = true;

		/// <summary>
		/// The repository to use for holding recent selected records. When populated, recents will be used, otherwise recent functionality will be disabled. Requires the <see cref="KeyField"/> to be populated with a valid unique key for <typeparamref name="TItem"/>.
		/// </summary>
		[Parameter] public IRecentRepository? RecentRepository { get; set; }

		/// <summary>
		/// When present, is the field containing a unique key for the <typeparamref name="TItem"/>.
		/// </summary>
		...
		/// <summary>
		/// The debounce delay in milliseconds between filtering results during typing. Defaults to <c>0</c>.
		/// </summary>
		[Parameter] public int DebounceMilliseconds { get; set; } = 0;

		/// <summary>
		/// The initialization delay in milliseconds. Occurs before initializing the list of search items. Defaults to <c>100</c>.
		/// </summary>
		[Parameter] public int InitializationDelayMilliseconds { get; set; } = 100;

		/// <summary>
		/// When true, the currently selected item in the list will be chosen when the Tab key is pressed. Defaults to <c>false</c>.
		/// </summary>
		[Parameter] public bool ChooseItemOnTab { get; set; }

		/// <summary>
		/// When true, the currently selected item in the list will be chosen on blur. Defaults to <c>false</c>.
		/// </summary>
		[Parameter] public bool ChooseItemOnBlur { get; set; }

		/// <summary>
		/// When true, search items will only populate the element is in focus. Defaults to <c>false</c>.
		/// </summary>
